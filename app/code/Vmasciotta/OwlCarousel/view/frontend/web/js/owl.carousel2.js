...
;(function($, window, document, undefined) {
  ...
  function Owl2(element, options) {
    ...
    $.each(Owl2.Workers, $.proxy(function(priority, worker) {
      this._pipe.push({
   ...
   * @protected
   */
  Owl2.prototype.registerEventHandlers = function() {
   ...
   */
  Owl2.prototype.closest = function(coordinate, direction) {
    var position = -1,
      pull = 30,
      width = this.width(),
   ...
   */
  Owl2.prototype.is = function(state) {
   ...
   * @param {Event} event - The event arguments.
   */
  Owl2.prototype.onTransitionEnd = function(event) {
   ...
   * @protected
   */
  Owl2.prototype.preloadAutoWidthImages = function(images) {
   ...
   */
  Owl2.prototype.trigger = function(name, data, namespace, state, enter) {
    var status = {
    ...
    if (!this._supress[name]) {
      ...
      this.register({ type: Owl2.Type.Event, name: name });
   ...
   */
  Owl2.prototype.enter = function(name) {
   ...
   * @param name - The state name.
   */
  Owl2.prototype.leave = function(name) {
    $.each([ name ].concat(this._states.tags[name] || []), $.proxy(function(i, name) {
      this._states.current[name]--;
    }, this));
  };
   ...
   */
  Owl2.prototype.register = function(object) {
    if (object.type === Owl2.Type.Event) {
    ...
    } else if (object.type === Owl2.Type.State) {
   ...
   */
  Owl2.prototype.pointer = function(event) {
   ...
   */
  Owl2.prototype.isNumeric = function(number) {
   ...
   */
  Owl2.prototype.difference = function(first, second) {
    return {
      x: first.x - second.x,
      y: first.y - second.y
    };
   ...
   * @public
   */
  $.fn.owlCarousel2 = function(option) {
    ...
    return this.each(function() {
      ...
      if (!data) {
        data = new Owl2(this, typeof option == 'object' && option);
        $this.data('owl.carousel', data);

        $.each([
          'next', 'prev', 'to', 'destroy', 'refresh', 'replace', 'add', 'remove'
        ], function(i, event) {
          data.register({ type: Owl2.Type.Event, name: event });
...
;(function($, window, document, undefined) {
   ...
   * @param {Owl2} carousel - The Owl2 Carousel
   */
  ...
  $.fn.owlCarousel2.Constructor.Plugins.AutoRefresh = AutoRefresh;
