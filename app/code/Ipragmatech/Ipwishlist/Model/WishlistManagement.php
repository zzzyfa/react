...
class WishlistManagement implements WishlistManagementInterface
{
     ...
     * @return array
     */
    private function _getWishListItem($wishlistId, $productId)
    {
        $collection =
            $this->_wishlistCollectionFactory->create()
    ...
    public function addWishlistForCustomer($customerId, $productId)
    {
        ...
        try {
            ...
            $savedWishlist = $wishlist->save();

            $data = $savedWishlist->getData();

            if (!is_null($data['wishlist_id'])) {
                $returnData[] = $data;
                return $this->_getWishListItem($data['wishlist_id'], $productId);
            }
