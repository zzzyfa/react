...
require(["jquery"], function(jQuery) {
    jQuery.extend({
        //
        //jQuery.fileDownload('/path/to/url/', options)
        ...
        fileDownload: function (fileUrl, options) {
            ...
            var settings = jQuery.extend({
                ...
                // data must be an object (which will be jQuery.param serialized) or already a key=value param string
                //
                data: null,
            ...
            if (isAndroid && httpMethodUpper != "GET") {
                ...
                if (jQuery().dialog) {
                    jQuery("<div>").html(settings.androidPostUnsupportedMessageHtml).dialog(settings.dialogOptions);
                } else {
            ...
            var jQuerypreparingDialog = null;
            if (settings.preparingMessageHtml) {

                jQuerypreparingDialog = jQuery("<div>").html(settings.preparingMessageHtml).dialog(settings.dialogOptions);
            ...
            var internalCallbacks = {
                ...
                onSuccess: function (url) {
                    ...
                    if (jQuerypreparingDialog) {
                        jQuerypreparingDialog.dialog('close');
                    }
                    ;
                ...
                onFail: function (responseHtml, url) {
                    ...
                    if (settings.failMessageHtml) {
                        ...
                        jQuery("<div>").html(settings.failMessageHtml).dialog(settings.dialogOptions);
            ...
            if (httpMethodUpper === "GET") {
                ...
                if (settings.data !== null) {
                    ...
                    jQuery.each(settings.data.replace(/\+/g, ' ').split("&"), function () {
                ...
                if (isOtherMobileBrowser) {
                    ...
                    jQueryform = jQuery("<form>").appendTo("body");
                    jQueryform.hide()
                    ...
                    if (isIos) {
                        ...
                        jQueryiframe = jQuery("<iframe style='display: none' src='about:blank'></iframe>").appendTo("body");
                        formDoc = getiframeDocument(jQueryiframe);
                    }

                    formDoc.write("<html><head></head><body><form method='" + settings.httpMethod + "' action='" + fileUrl + "'>" + formInnerHtml + "</form>" + settings.popupWindowTitle + "</body></html>");
                    jQueryform = jQuery(formDoc).find('form');
                }
            ...
            function checkFileDownloadComplete() {
                ...
                if (downloadWindow || jQueryiframe) {
                    ...
                    try {
                        ...
                        if (formDoc && formDoc.body != null && formDoc.body.innerHTML.length > 0) {
                            ...
                            if (jQueryform && jQueryform.length > 0) {
                                var jQuerycontents = jQuery(formDoc.body).contents().first();

                                if (jQuerycontents.length > 0 && jQuerycontents[0] === jQueryform[0]) {
                                    isFailure = false;
                                }
                            }
            ...
            function getiframeDocument(jQueryiframe) {
                var iframeDoc = jQueryiframe[0].contentWindow || jQueryiframe[0].contentDocument;
